...
const fetchCharacters = async () =>{
    const results = await Promise.all(data.characters.map((character)=>
    axios.get(character)
    ))
    return (results.map((res)=> res.data.name));
}

fetchCharacters().then(val=>{setChar(val)})

let charNames = <ul className="details-ul"> {char.map(names=><li className="details">{names}</li>)} </ul>

//planets

const fetchPlanets = async () =>{
    const results = await Promise.all(data.planets.map((planet)=>
    axios.get(planet)
    ))
    return (results.map((res)=> res.data.name));
}

fetchPlanets().then(val=>{setPlan(val)})

let planNames = <ul className="details-ul"> {plan.map(names=><li className="details">{names}</li>)} </ul>

//starships

const fetchStarships = async () =>{
    const results = await Promise.all(data.starships.map((starship)=>
    axios.get(starship)
    ))
    return (results.map((res)=> res.data.name));
}

fetchStarships().then(val=>{setShip(val)})

let shipNames = <ul className="details-ul"> {ship.map(names=><li className="details">{names}</li>)} </ul>

//vehicles

const fetchVehicles = async () =>{
    const results = await Promise.all(data.vehicles.map((vehicle)=>
    axios.get(vehicle)
    ))
    return (results.map((res)=> res.data.name));
}

fetchVehicles().then(val=>{setVeh(val)})

let vehicleNames = <ul className="details-ul"> {veh.map(names=><li className="details">{names}</li>)} </ul>

//species
const fetchSpecies = async () =>{
    const results = await Promise.all(data.species.map((specie)=>
    axios.get(specie)
    ))
    return (results.map((res)=> res.data.name));
}

fetchSpecies().then(val=>{setSpec(val)})

let specieNames = <ul className="details-ul"> {spec.map(names=><li className="details">{names}</li>)} </ul>
...
if(data){
  ...
  { <div className='more-content'> 
     ...
     <button onClick={()=> navigate(-1)}> Back to list</button> 
            ...
            <p className='moreopening-crawl'>{data.opening_crawl}</p>  

            <div className="characters">
            <h3 className='description'>Characters</h3> 
            {charNames}
            </div>

            <div className="planets">
            <h3 className='description'>Planets</h3> 
                {planNames}
            </div>
            ...
            <h3 className='description'>Starships</h3> 
                {shipNames}
            </div>

            <div className="vehicles">
            <h3 className='description'>Vehicles</h3> 
                {vehicleNames}
            </div>
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 612 612" width="512" height="512" className='spinner'>
<g>
